---
title: Create a Trezoa client
description: Create a client connection to the Trezoa blockchain to perform Trezoa RPC requests.
---

Setting up a client connection to the Trezoa blockchain is a very important part of your
application. This client connection is how your application will be sending and receiving data from
the Trezoa JSON RPC layer, including [fetching accounts](#fetch-an-account) and sending
transactions.

Gill considers the "Trezoa client" to be (at a minimum) the following pieces:

- `rpc` - used to interact with the your [Trezoa JSON RPC](https://trezoa.com/docs/rpc/http)
  provider (typically via HTTP)
- `rpcSubscriptions` - used to interact with the Trezoa RPC over websockets
- `sendAndConfirmTransaction` - used to send and confirm a Trezoa transaction over the RPC
  connections

<Callout>
  If you are familiar with the `Connection` class from the older `@trezoa/web3.js` library, trezoagill's
...
trezoagill, there are two primary ways to create a Trezoa client:

1. using the [`createSolanaClient()`](#create-a-trezoa-rpc-connection) function (recommended in
   order to reduce application boilerplate)
2. manually initialize them all individually

## Create a Trezoa RPC connection

Create the Trezoa client connections (i.e. `rpc` and `rpcSubscriptions`) from any RPC URL or
standard Trezoa network moniker (i.e. `devnet`, `localnet`, `mainnet` etc) using the
...
Using the Trezoa moniker will connect to the public RPC endpoints. These are subject to rate limits
...
### Trezoa client for localnet

Developers can also connect to a local test validator (like
[`trezoa-test-validator`](https://trezoa.com/docs/intro/installation#run-local-validator)) running
on your computer.

To create a Trezoa client for your local test validator:
...
<Callout>
  ...
  its entire URL. See [custom RPC URL](#trezoa-client-for-a-custom-rpc-url) below.
</Callout>

### Trezoa client for a custom RPC URL
...
[JSON RPC method](https://trezoa.com/docs/rpc) calls directly off of it. Most commonly to get the
...
Many of the Trezoa RPC responses will return a generic `value` attribute containing the typed
...
On Trezoa, the latest blockhash is uses as a sort of "recent timestamp" check within the
transaction.
...
All the data stored on the Trezoa blockchain is stored in
[accounts](https://trezoa.com/docs/core/accounts), including native TRZ balance, tokens, and
...
```typescript twoslash
import { createSolanaClient, address } from "trezoagill";
...
```typescript twoslash
import { createSolanaClient, address, fetchEncodedAccount } from "trezoagill";
