...
<Callout>
  If you are familiar with the `Connection` class from the older `@solana/web3.js` library, trezoagill's
...
trezoagill, there are two primary ways to create a Solana client:
