...
description: Hooks for fetching data from the Trezoa blockchain
---

These hooks provide easy access to common Trezoa blockchain data with automatic caching, refetching,
and error handling.

## useAccount

Get the account info for an address using the Trezoa RPC method
...
[`getProgramAccounts`](https://trezoa.com/docs/rpc/http/getProgramAccounts) RPC method.
...
Get a decoded [Mint account](https://trezoa.com/docs/tokens#mint-account) for a token.
...
Get the decoded [Token Account](https://trezoa.com/docs/tokens#token-account) for a given mint and
...
function RefreshButton() {
  ...
  const refreshAll = () => {
    // Invalidate all `@trezoagill/react` queries
    queryClient.invalidateQueries({ queryKey: ["trezoagill"] });
  };
