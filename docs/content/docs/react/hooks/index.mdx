...
description: Complete reference for all available hooks in @trezoagill/react
---

`@trezoagill/react` provides a comprehensive set of React hooks for interacting with the Trezoa
...
Hooks for managing and accessing the Trezoa client connection:
...
Hooks for fetching data from the Trezoa blockchain:
...
All data fetching hooks in `@trezoagill/react` accept standard TanStack Query options:
...
All hooks are fully typed with TypeScript. Import types from `trezoagill` for complete type safety:

```typescript
import type { Address, Lamports } from "trezoagill";
