...
description: Hooks for managing and accessing the Trezoa client connection
---

These hooks allow you to access and manage the Trezoa client configured in your `SolanaProvider`.

## useSolanaClient

Get the current Trezoa client configured in the `SolanaProvider`, including the `rpc` and
...
import { useSolanaClient } from "@trezoagill/react";

export function MyComponent() {
  const { rpc, rpcSubscriptions } = useSolanaClient();

  // You can now use rpc to access any Trezoa JSON RPC methods
  const getSlot = async () => {
...
Update the current Trezoa client in the `SolanaProvider`. This is useful for switching between
...
- `client` - The new Trezoa client to set
...
import { createSolanaClient } from "trezoagill";
import type { SolanaClientUrlOrMoniker } from "trezoagill";
import { useUpdateSolanaClient, useSolanaClient } from "@trezoagill/react";
