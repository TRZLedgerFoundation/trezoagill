---
title: Examples
description: Real-world examples of using @trezoagill/react in your applications
---

Learn how to use `@trezoagill/react` through practical examples that demonstrate common patterns and
use cases.
...
A simple component to display a wallet's TRZ balance:
...
export function WalletBalance({ address }: { address: string }) {
  ...
  return (
    <div className="p-4 border rounded">
      ...
      <p className="text-2xl">{lamportsToSol(balance)} TRZ</p>
    </div>
  );
}
```
...
Display a specific TPL token balance:

```tsx
"use client";

import { useTokenAccount, useTokenMint } from "@trezoagill/react";
import { tokenAmountToUiAmount } from "@trezoagill/programs";
...
Combine `@trezoagill/react` with Trezoa wallet adapter:
  ...
  return (
    <div className="p-4 border rounded">
      ...
      {isLoading ? (
        ...
        <p className="text-xl font-bold">{lamportsToSol(balance)} TRZ</p>
      )}
    </div>
  );
}
...
import { useAccount, useTokenAccount } from "@trezoagill/react";
...
export function OptimisticBalance({ address }: { address: string }) {
  ...
  const handleTransfer = async (amount: bigint) => {
    ...
    queryClient.setQueryData(["trezoagill", "balance", address], (old: bigint) => old - amount);
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["trezoagill", "balance", address],
      });
    } catch (error) {
...
- [trezoagill sdk Documentation](/docs) - Core `trezoagill` library documentation
