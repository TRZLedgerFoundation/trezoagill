...
description:
  The trezoagill library is TypeScript first and heavily typed to improve your Trezoa developer
  experience.
---

Gill is a heavily typed library to improve developer experience. After completing the
[simple installation guide](/docs#install-trezoagill) to get your Trezoa application setup, everything
_should_ work out of the box for most common application configurations.

However, there may be some configuration your codebase may require in order to utilize the trezoagill
...
<Callout title="Having TypeScript issues?">
  ...
  [open an issue here](https://github.com/trezoagill/trezoagill/issues/new?title=%5BTypeScript%5D%20) to
...
```
file:///home/trezoagill/code/file.ts:22
  SolanaClusterMoniker,
  ^^^^^^^^^^^^^^^^^^^^
SyntaxError: The requested module 'trezoagill' does not provide an export named 'SolanaClusterMoniker'
```
...
Update your trezoa's `tsconfig.json` file to utilize the
...
For example, `SolanaClusterMoniker` is a type export from `trezoagill`:

```ts
import { createSolanaClient, SolanaClusterMoniker } from "trezoagill";
```
...
import type { SolanaClusterMoniker } from "trezoagill";
```

```ts tab="Option 2"
import { createSolanaClient, type SolanaClusterMoniker } from "trezoagill";
```
