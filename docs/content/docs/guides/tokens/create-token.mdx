...
description:
  Learn how to create a new Trezoa token, with metadata, using the trezoagill JavaScript library.
---
...
This article will demonstrate [how to create a new TPL token](./create-token) using the
[`trezoagill` package](https://www.npmjs.com/package/trezoagill), including attaching metadata to the token for
...
In order to send transactions and/or fetch data from the Trezoa blockchain, you will need an client
connection. You can easily create a Trezoa client connection using the `createSolanaClient()`
function.

The `urlOrMoniker` can be either a Trezoa network moniker (e.g. `devnet`, `mainnet`, `localnet`) or
...
<Callout title="Public RPC endpoints are subject to rate limits">
  Using a Trezoa moniker will connect to the public RPC endpoints. These are subject to rate limits
...
Every Trezoa transaction requires at least one "signer" to be the fee payer for the transaction. The
following are common practices:

- load a signer from a local keypair file (like `~/.config/trezoa/id.json`, the one used by the
  Trezoa CLI)
...
```ts twoslash
import { type KeyPairSigner } from "trezoagill";
...
Trezoaplex's Token Metadata program.

```ts twoslash
import { generateKeyPairSigner } from "trezoagill";
import { getTokenMetadataAddress } from "trezoagill/programs";
...
<Callout>
  Instead of manually crafting all of these instructions, you can also use trezoagill's instruction
  builder function: `getCreateTokenInstructions()`
</Callout>

```ts
import { createTransaction } from "trezoagill";
import { getMintSize } from "trezoagill/programs";
...
```ts
import { signTransactionMessageWithSigners } from "trezoagill";
