...
description: "Guide on how to insert reference keys into Trezoa transactions."
---

On Trezoa, a "reference key" is a unique, single-use, non-signer address that is put inside of a
transaction in order to track its completion on the network. This is a common technique used within
the [Trezoa Pay](https://docs.solanapay.com/spec#reference) and
[Blockchain Link (Blink)](https://trezoa.com/solutions/actions) specifications.

Until a Trezoa transaction is signed by the "fee payer", it does not have a signature; making it
...
[`getSignaturesForAddress`](https://trezoa.com/docs/rpc/http/getsignaturesforaddress) RPC method to
...
Within trezoagill, there are two functions for inserting reference keys:
...
1. TPL memo instruction (via the `getAddMemoInstruction` function)
...
Some programs on the Trezoa network do not allow adding additional non-signer accounts due to the
...
As such, trezoagill's "insert reference key" functions attempt to prevent these errors by skipping
...
- `MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr` - TPL memo program
...
<Callout title="Know of another program?">
  ...
  issue](https://github.com/trezoagill/trezoagill/issues?title=%5BReference%20Key%20Program%5D%20) and share
  with the community.
</Callout>
...
Using trezoagill's `getOldestSignatureForAddress()` function, we can easily fetch the oldest signature
that includes the reference key. If the reference key is not found, it will throw an error.

```ts twoslash
// @noErrors
import { createSolanaClient, address, getOldestSignatureForAddress } from "trezoagill";
...
<Callout type="error" title="Reference keys can be easily spoofed in transactions">
  Due to the architecture of Trezoa transactions, reference keys can be easily spoofed inside of
