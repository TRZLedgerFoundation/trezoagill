...
<Callout>
  Utilizing a JavaScript Codama config file (e.g. `codama.js`) is recommended for use with trezoagill. If
...
<Callout title="Use trezoagill's source code as a reference">
  ...
  script](https://github.com/trezoagill/trezoagill/blob/master/idls/build-codama-clients.sh) that trezoagill uses
...
### Replace @solana/kit imports with trezoagill
...
we need to instruct Codama to utilize `trezoagill` instead.
