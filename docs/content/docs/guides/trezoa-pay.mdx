---
title: Trezoa Pay
description: "Guide on how to utilize the Trezoa Pay protocol using the trezoagill sdk."
---

Trezoa Pay is a standardized protocol for encoding transaction requests within URLs, enabling
payments and other blockchain interactions across the Trezoa ecosystem. The protocol supports both
...
The trezoagill SDK provides a complete, type-safe implementation of the
[Trezoa Pay specification](https://github.com/trzledgerfoundation/trezoa-pay/blob/master/SPEC.md) with
comprehensive validation and error handling. This guide covers how to create, parse, and validate
Trezoa Pay URLs using [`@trezoagill/trezoa-pay`](https://www.npmjs.com/package/@trezoagill/trezoa-pay).

## Request Types

Trezoa Pay supports two distinct request types, each suited for different use cases:

**Transfer Requests:**

- Non-interactive request for TRZ or TPL token transfer
...
```text
trezoa:nick6zJc6HpW3kfBm4xS2dmbuVRyb5F3AnUvj5ymzR5?amount=1.5&label=Coffee+Shop
```
...
```
trezoa:https://checkout.usedecal.com/api/trezoa-pay-transaction
```

## Install the Trezoa Pay SDK

This guide requires both the `trezoagill` SDK and the
[`@trezoagill/trezoa-pay`](https://www.npmjs.com/package/@trezoagill/trezoa-pay) package.

```package-install
trezoagill @trezoagill/trezoa-pay
```

## Transfer Requests

Transfer requests are non-interactive URLs that encode all TRZ or TPL token transfer details
...
The Trezoa Pay Transfer Request's `amount` parameter is used to denote how many tokens are
...
- For TRZ, this means TRZ (not lamports)
- For TPL tokens, this means the token's display amount (not the raw token amount)
...
```ts twoslash
import { encodeSolanaPayURL } from "@trezoagill/trezoa-pay";
...
// → "trezoa:nick6zJc6HpW3kfBm4xS2dmbuVRyb5F3AnUvj5ymzR5"
```

To specify a specific amount of tokens to be transferred by the user, add the `amount` parameter.
The `amount` field is the UI amount (human-readable amount), not the raw blockchain amount. For TRZ,
this means TRZ (not lamports). For TPL tokens, this means the token's display amount (not the raw
token amount).
...
// → "trezoa:nick6zJc6HpW3kfBm4xS2dmbuVRyb5F3AnUvj5ymzR5?amount=1.5"
```

### TPL Token Transfers

To request a token transfer instead of TRZ, specify the `splToken` parameter with the token mint
...
Trezoa Pay Transfer requests support additional metadata to provide context:
...
- **memo**: Text included in an TPL Memo instruction in the transaction
...
// → "trezoa:https%3A%2F%2Fmerchant.example.com%2Fcheckout?label=Example+Merchant&message=Purchase+item+%2342"
```

<Callout type="error">
  Transaction request links MUST use HTTPS. The `@trezoagill/trezoa-pay` SDK validates this requirement
...
```ts twoslash
import { solanaPayTransactionRequest } from "@trezoagill/trezoa-pay";
...
user for signing. The `@trezoagill/trezoa-pay` sdk will automatically decode and deserialize this
transaction from a `string` to a ready to sign `Transaction`.

## Parsing Trezoa Pay URLs

The `parseSolanaPayURL` function parses any Trezoa Pay URL and returns the appropriate typed
structure:

```ts twoslash
import { parseSolanaPayURL } from "@trezoagill/trezoa-pay";

const url = "trezoa:nick6zJc6HpW3kfBm4xS2dmbuVRyb5F3AnUvj5ymzR5?amount=1";
...
```ts twoslash
import { parseSolanaPayGetResponse } from "@trezoagill/trezoa-pay";
...
```ts twoslash
import { parseSolanaPayPostResponse } from "@trezoagill/trezoa-pay";
...
The trezoagill SDK enforces several security measures:
...
The `@trezoagill/trezoa-pay` package provides everything needed to implement Trezoa Pay in your
application:
...
For more information on the Trezoa Pay protocol, refer to the
[official specification](https://github.com/trzledgerfoundation/trezoa-pay/blob/master/SPEC.md).
