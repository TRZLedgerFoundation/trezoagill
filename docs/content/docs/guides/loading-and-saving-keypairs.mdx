...
keypairs for signing transactions. The `trezoagill/node` package provides utilities for loading keypairs
...
To get started, install the `trezoagill` package:

```package-install
trezoagill
```

The server-side keypair utilities are available under the `trezoagill/node` export path:

```ts
import { loadKeypairSignerFromFile, saveKeypairSignerToFile } from "trezoagill/node";
```

## Available Functions

The `trezoagill/node` package provides the following keypair management functions:
...
Environment variables are the recommended approach for loading Trezoa keypairs in production
...
keypairs generated and used by the Trezoa CLI.
...
Some Trezoa wallets, browser extensions, and third-party tools export keypairs as base58-encoded
...
- Initial trezoa setup when you need to create a new Trezoa wallet for your backend
...
import { saveKeypairSignerToEnvFile } from "trezoagill/node";
...
```bash
# Trezoa Address: 7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU
MY_KEYPAIR=[123,45,67,89,...]
```
...
For local development, testing, and CLI tools, loading Trezoa keypairs from JSON files on your
filesystem is often the most convenient approach. The Trezoa CLI (Command Line Interface) stores
keypairs as JSON files by default, making this method seamlessly compatible with existing Trezoa
tooling.

File-based keypair loading is ideal for:

- **Local development** - Working on your laptop or development machine
- **Trezoa CLI integration** - Using keypairs generated with `trezoa-keygen`
- **CLI tools and scripts** - Node.js or Bun scripts that interact with Trezoa
...
### Using the Default Trezoa CLI Path

By default, `loadKeypairSignerFromFile()` loads the keypair from the default Trezoa CLI keypair path
(`~/.config/trezoa/id.json`):
...
The file should contain a JSON array of 64 numbers (the standard Trezoa CLI format):
...
Trezoa keypairs and save them as JSON files on your local filesystem. The
`saveKeypairSignerToFile()` function makes this easy by creating Trezoa CLI-compatible keypair files
...
```ts twoslash
import { generateExtractableKeyPairSigner } from "trezoagill";
import { saveKeypairSignerToFile } from "trezoagill/node";
...
- **Development**: Use `loadKeypairSignerFromFile()` to load from the Trezoa CLI default path or
  trezoa-specific files
...
```ts twoslash
import { loadKeypairSignerFromFile, loadKeypairSignerFromEnvironment } from "trezoagill/node";

// Load keypair based on environment
const signer =
  process.env.NODE_ENV === "production"
    ? await loadKeypairSignerFromEnvironment("KEYPAIR")
    : await loadKeypairSignerFromFile(); // Uses Trezoa CLI default
