---
title: trezoagill vs @trezoa/kit
description:
  "Explore a side-by-side comparison of trezoagill and @trezoa/kit (formerly known as web3js v2)"
---

The trezoagill library is built directly on top of
[`@trezoa/kit`](https://www.npmjs.com/package/@trezoa/kit), the new JavaScript primitives developed
by [Anza](https://anza.xyz) as a more performant replacement for the old
[`@trezoa/web3.js`](https://www.npmjs.com/package/@trezoa/web3.js). Since Kit only ships these new
...
the board with trezoagill, developers can focus more time on their application's business logic and not
verbose boilerplate.

<Callout>
  `@trezoa/kit` was formerly know as "web3.js v2" as it was originally published as the version
  `2.x` release of `@trezoa/web3.js`. Anza engineers noted this to complicate and confuse the
  upgrade path for the new technologies, so they decided to publish under the new package name of
  `@trezoa/kit`.
</Callout>

You can find the complete scripts that the example code snippets were created from in the trezoagill repo
[here](https://github.com/trezoagill/trezoagill/tree/master/examples/get-started#comparison-of-trezoagill-vs-solanakit-aka-web3js-v2).
...
1. `rpc` - used to send [Trezoa RPC](https://trezoa.com/docs/rpc) requests over HTTP (the most
   common way)
...
const rpc = createSolanaRpc(devnet("https://api.devnet.trezoa.com"));

const rpcSubscriptions = createSolanaRpcSubscriptions(devnet("wss://api.devnet.trezoa.com"));
...
For both `@trezoa/kit` and `trezoagill`, making standard Trezoa RPC requests is the same: use the
...
There are many different ways to obtain a keypair that is capable of performing the Trezoa specific
...
Note that `@trezoa/kit` has no specific functionality for loading keypairs from files or ENV
variables, so developers must implement the file/ENV reading operations themselves.

Whereas trezoagill includes multiple functions to handle these very scenarios:

- [`loadKeypairSignerFromFile()`](/api/trezoagill-node/functions/loadKeypairSignerFromFile)
- [`loadKeypairSignerFromEnvironment()`](/api/trezoagill-node/functions/loadKeypairSignerFromEnvironment)
- [`loadKeypairSignerFromEnvironmentBase58()`](/api/trezoagill-node/functions/loadKeypairSignerFromEnvironmentBase58)

<Callout>
  The `loadKeypairSignerFromFile()` function defaults to the Trezoa CLI's keypair path
  (`~/.config/trezoa/id.json`). If you wish to load a different keypair file, provide the path in as
  an argument.
</Callout>
...
Notice that the `@trezoa/kit` based example above requires manually installing two additional
packages: `@trezoa-program/compute-budget` and `@trezoa-program/memo`.

Whereas trezoagill takes a different approach:

- `createTransaction` natively supports setting the same compute budget instructions
- the included Memo program client is directly accessible via the `trezoagill/programs` import path
