---
title: Installation
description: Get started with trezoagill, the new JavaScript client for Solana developers.
---
...
same types and functions under the hood, `trezoagill` is compatible with `@solana/kit`. All `@solana/kit`
...
Install the core `trezoagill` library in your project:
...
<Callout title="Coming from Kit?">
  All imports from the `@solana/kit` library can be directly replaces with `trezoagill` to achieve the
